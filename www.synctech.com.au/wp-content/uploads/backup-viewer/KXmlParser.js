...
self.addEventListener('unhandledrejection', function (event) {
    ...
    throw event.reason + ".<br/>Stack trace: " + event.reason.stack;
});
...
async function loadAllForCache(file, sortOrder, loadMedia, startDate, endDate) {
    var parser = new KXmlParser();
    parser.setIgnoreBinaryData(!loadMedia);
    parser.setInput(file);
    var eventType = parser.getEventType();
    var contacts = [];
    var recordCount = 10000;
...
function readBinaryData(parser, fileName, contentType, messageIndex, partIndex) {
    if (contentType.indexOf("image") == 0) {
        ...
        postMessage({ name: "BINARY_DATA", data: parser.readBinaryDataString() });
...
class KXmlParser {
    constructor() {
        ...
        /**
         * Buffer for the current element being read
         */
        ...
        */
        ...
        this.depth = 0;
        /**
         * The current position in the srcBuf where the next character will be read from
         */
        this.srcPos = 0;
        /**
         * The number of characters that have been read into srcBuf
         */
        this.srcCount = 0;
        /**
         * The position where the file reader will read next from
         */
        ...
        this.line = 0;
        /**
         * The column on the current line in the file
         */
        this.column = 0;
        /**
         * The position in txtBuf that is being added
         */
        this.txtPos = 0;
        ...
        /**
         * Buffer for processing the current chunk of data that has been read from the file
         */
    ...
    */
    nextImpl() {
        ...
        if (this.type === KXmlParser.START_TAG && this.mFoundBinaryData) {
            // We found the binary data but we've been asked to read the next attribute, so lets skip to the end
            this.skipToEndOfBinaryData();
            this.mFoundBinaryData = false;
        }
        ...
        while ((true)) {
            ...
            if (this.peek(0) !== ('=').charCodeAt(0)) {
                ...
                if (attrName === "data") {
                    if (this.mIgnoreBinaryData) {
                    ...
                    else if (this.mDeferBinaryDataProcessing) {
                        // Mark that we've found the binary data. The nextImpl() can then check if it needs to skip or read it
                        ...
                        this.attributes[i] = this.readBinaryDataString();
                    }
                }
                else {
    ...
    */
    readFromFile() {
        ...
        this.currentReadPosition = readEndPosition;
    ...
    readAndSendBinaryDownloadDataToStream(contentType, messageIndex, partIndex, fileName) {
        var base64Content = this.readBinaryDataString();
        postMessage({ name: "BINARY_DATA_DOWNLOAD", data: "data:" + contentType + ";base64," + base64Content, file_name: fileName });
        postMessage({ name: "BINARY_DATA_DOWNLOAD_INITIATED", data: messageIndex + "_" + partIndex });
    }

    /**
     *  Loads the content of the data attribute directly into a string, rather than parsing each character
     */
    readBinaryDataString() {
        var stringRead = "";

        // Find the start position of the attribute in the file and read till we find the end double-quotes
        var readStartPosition = this.srcPos;
        if (this.currentReadPosition > this.srcCount) {
            readStartPosition += this.currentReadPosition - this.srcCount;
        } 
        do {
            var readEndPosition = readStartPosition + this.srcBufferSize;
            if (readEndPosition > this.contentSize) {
                readEndPosition = this.contentSize;
            }
            var blob = this.currentFile.slice(readStartPosition, readEndPosition);
            var textRead = this.fileReader.readAsText(blob);
            var indexOfEndQuotes = textRead.indexOf('\"');
            if (indexOfEndQuotes > -1 || readEndPosition >= this.contentSize) {

                // Found the quotes, that means the end of the attribute
                // Append the last bit of the attribute
                stringRead = stringRead.concat(textRead.substring(0, indexOfEndQuotes))

                // Reset the current start position so the rest of the data is read after the end of the binary data
                this.currentReadPosition = readStartPosition + indexOfEndQuotes + 1;

                // Adjust the column position, assuming the whole binary data is on 1 single line
                this.column += stringRead.length + 1;

                // set the start of attribute to 0
                this.txtPos = 0;

                // set the start position of read buffer higher than the current buffer count so it reads a new chunk from the file
                this.srcPos = this.currentReadPosition;
                return stringRead;
            }
            else {
                // Append the read data and move to the next chunk
                stringRead = stringRead.concat(textRead);
                readStartPosition = readStartPosition + textRead.length;
            }
        } while (true);
    }

    /**
    * Skips to the end of the binary data so that the next attribute can be read.
    * This is faster than normal skipTo call as it doesn't parse individual characters.
    */
    skipToEndOfBinaryData() {
        var binaryDataCount = 0;
        ...
        do {
            ...
            if (indexOfEndQuotes > -1 || readEndPosition >= this.contentSize) {
                ...
                // Found the quotes, that means the end of the attribute
                // Count the last bit of the attribute
                binaryDataCount += indexOfEndQuotes;

                // Reset the current start position so the rest of the data is read after the end of the binary data
                this.currentReadPosition = readStartPosition + indexOfEndQuotes + 1;

                // Adjust the column position, assuming the whole binary data is on 1 single line
                this.column += binaryDataCount + 1;
                ...
                // Count the read data and move to the next chunk
                binaryDataCount += textRead.length;
                readStartPosition = readStartPosition + textRead.length;
            }
        } while (true);
    }
